...
import { addIssue, removeIssue } from "../utils/issues";
...
export interface IMongoService {
  addKYC(userId: ID, KYCData: IKYCData): Promise<IDBResponse>;
  ...
  createInvestment(
    investmentDetails: IInvestmentReq
  ): Promise<[IInvestment, ITransaction]>;
  createTransaction(TransactionDetails: ITransactionReq): Promise<ITransaction>;
  ...
  verifyDeposit(
    ...
    IDBResponse | { firstName: string; email: string; inv: IInvestment }
  >;
...
export class MongoService implements IMongoService {
  async addKYC(userId: ID, KYCData: IKYCData) {
    ...
    dash.issues = addIssue({ array: dash.issues, element: "KYC" });
    await dash.save();
  ...
  async confirmWithdrawal(transId: ID) {
    ...
    try {
      ...
      // }
      dash.issues = removeIssue({ array: dash.issues, element: "withdrawal" });
      await dash.save();
    } catch ({ message }) {
  ...
  async createTransaction(transactionDetails: ITransactionReq) {
    const dashB = await this.findDashboard(transactionDetails.investor);
    if (!dashB) {
  ...
  async createInvestment(
    ...
    dashB.issues = addIssue({ array: dashB.issues, element: "Deposit" });
  ...
  async getAdminMail() {
    const { email } = <IUser>(
      await User.findOne({ role: "ADMIN" }).select("email").exec()
    );
    return email;
  }
  ...
  async getAllDashboards() {
    ...
    const populatedDashboards = await Promise.all(
      dashboards.map(async (dashboard) => {
        ...
        if (dashboard.hasInvestment) {
          ...
          if (dashboard.investment) {
            ...
            const trans = await Transaction.findById(
  ...
  async verifyDeposit(invId: ID) {
    ...
    const dash = await this.findDashboard(<ID>inv.investor);
    if (!dash) return { statusCode: 500, message: "Investment not found" };

    inv.status = STATUS.ACTIVE;
    await inv.save();
    await inv.populate("transaction");

    const { statusCode, message } = await this.confirmTransaction(
      <ID>inv.transaction
    );
    if (statusCode !== 200) return { statusCode, message };

    dash.issues = removeIssue({ array: dash.issues, element: "Deposit" });
    await dash.save();

    const { email, firstName } = <IUser>(
      await this.findUserById(<ID>inv.investor)
    );
  ...
  async verifyUser(userId: ID) {
    ...
    try {
      ...
      dash.issues = removeIssue({ array: dash.issues, element: "KYC" });
      await dash.save();
